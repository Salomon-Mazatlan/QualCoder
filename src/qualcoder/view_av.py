...
from .html_parser import html_to_text  # Homologate transcript formats with Manage files <- L
...
class DialogViewAV(QtWidgets.QDialog):
    ...
    def __init__(self, app, file_, parent=None):
        ...
        self.code_deletions = []
        self.time_positions = []
        self.speaker_list = []  # loaded from / persisted to speakers.json <- L
        self.speaker_formats = {}  # per-speaker identifier <- L

        QtWidgets.QDialog.__init__(self)
        self.ui = Ui_Dialog_view_av()
        self.ui.setupUi(self)
        # Main splitter is horizontal: video/speakers/snippets left, writing + waveform right. <- L
        ...
        # All section layouts persist, as in code_text. <- L
        try:
        ...
        # Migrates once from the old config.ini key. <- L
        ...
        # Predefined elements: symbols only (description in the tooltip); speakers first. <- L
        ...
        # Line numbers in the transcription area. <- L
        LineNumberArea.install(self.ui.textEdit)
        ...
        if self.file_['av_text_id'] is not None:
            ...
            if self.transcription is not None and self.transcription[1] is None:
                # Old projects can hold NULL fulltext; normalise so setText/regex do not crash <- L
        ...
        if self.transcription is None:
            ...
            if res is None:
                ...
                # av_text_id link could be lost, recreating duplicate .txt transcripts <- L
        ...
        # Esc reject() hides the window while detached, leaving no video target: dock it back. <- L
        ...
        if not self.is_audio:
            # Ensure the video pane is actually visible: a collapsed splitter pane
            # (size 0 persisted from an audio session or a manual drag) stays 0 and
            # the preview never shows for the next video. <- L
    ...
    def _on_ddialog_closed(self):
        """ The floating window was dismissed (Esc): dock the video back. <- L """
        if getattr(self, 'video_detached', False):
    ...
    def load_transcription(self):
        ...
        if not filename:
            return
        try:
            # Same formats and reading as Manage files > Import transcription from file. <- L
        ...
        if text is None:
            text = ""
        # Normalise line endings and strip BOM so stored positions match the editor. <- L
        text = text.replace("\r\n", "\n").replace("\r", "\n")
        if text and text[0] == "\ufeff":
            text = text[1:]
        if text.strip() == "":
            ...
            return
        # Apply pseudonyms, consistent with the Manage files transcript import. <- L
        ...
        if self.ui.textEdit.toPlainText().strip():
            # Warn with concrete counts, same as the Manage files import <- L
        ...
        # old against new text and shift every coding/annotation. <- L
        self.ui.textEdit.blockSignals(True)
        self.ui.textEdit.setPlainText(text)
        self.ui.textEdit.blockSignals(False)
        self.text = text
        self.prev_text = copy(text)
        self.text_has_changed = False  # already persisted below, nothing pending for autosave <- L
        ...
        self.transcription = cur.fetchone()
        if self.transcription is not None and self.transcription[1] is None:
            self.transcription = (self.transcription[0], "", self.transcription[2])  # <- L
        self.get_timestamps_from_transcription()
        # Refresh coded text, annotations, cases and repaint over the new text <- L
        self.get_cases_codings_annotations()
        self.highlight()
    ...
    def _load_pseudonyms(self):
        ...
        circular import (manage_files already imports view_av). <- L
    ...
    def slider_seek(self, value):
        ...
        Routed through _seek_to_ms so slider, playhead and time label stay in sync. <- L """
    ...
    def eventFilter(self, object_, event):
        ...
        # Alt+Enter always inserts the next speaker; the checkbox makes every Enter do it. <- L
        ...
        if (key == QtCore.Qt.Key.Key_S or key == QtCore.Qt.Key.Key_P) and \
            ...
            self.play_pause()
        # Transport keys as in transcription software: F3 -5s, F4 play/pause, F5 +5s. <- L
        ...
        if key == QtCore.Qt.Key.Key_B and mods & QtCore.Qt.KeyboardModifier.ShiftModifier and \
            ...
            # overwrites the bookmark just visited (Ctrl is still held). <- L
        # Set bookmark
    ...
    def _speaker_snippet_text(self, speaker):
        """ Insert text for a speaker, using the identifier chosen at creation. <- L """
    ...
    def _load_speakers_json(self):
        """ Read the project speakers from speakers.json. <- L """
        try:
            ...
            if not speakers and isinstance(data, dict):
                # Migration: merge old per-file lists into the shared one. <- L
                merged = []
    ...
    def _save_speakers_json(self):
        """ Save the speakers to speakers.json, keeping other keys intact. <- L """
    ...
    def refresh_snippets_list(self):
        """ Rebuild the snippets list: speakers always first (bold), then symbols. <- L """
        ...
        for i, speaker in enumerate(self.speaker_list):
            # Show the speaker as it will be inserted; shortcut in the tooltip. <- L
    ...
    def snippets_menu(self, position):
        """ Snippets list context menu: edit speaker. <- L """
    ...
    def insert_snippet(self, item):
        """ Insert at the cursor; speakers use the identifier chosen at creation. <- L """
    ...
    def _insert_next_speaker(self):
        """ Insert the next speaker in the list (cyclic), in its own format. <- L """
        if not self.speaker_list:
            return
    ...
    def _speaker_dialog(self, initial_name="", initial_fmt=None, title=None):
        ...
        Returns (name, format), or None if cancelled or the name is invalid. <- L """
    ...
    def add_speakername(self):
        """ Add a speaker: one dialog with name + identifier. Maximum of 8. <- L """
    ...
    def edit_speaker(self, old_name):
        ...
        position (and so the Ctrl+n shortcut). <- L """
    ...
    def insert_speakername(self, key):
        ...
        try:
            ...
            return False
        # Uses the identifier chosen when the speaker was created. <- L
    ...
    def add_speaker_names_to_label(self):
        ...
        # Persist the list and mirror it in the snippets. <- L
        self._save_speakers_json()
        self.refresh_snippets_list()
    ...
    def get_timestamps_from_transcription(self):
        ...
        hhmmss_sss = r"#[0-9][0-9]:[0-9][0-9]:[0-9][0-9]\.[0-9]{1,3}#"  # 1-3 msec digits, same as the coder <- L
        ...
        for match in re.finditer(hhmmss3, transcription):
            # Format {00:34:20} is colon separated; splitting by '.' crashed on load. <- L
            stamp = match.group()[1:-1]
            s = stamp.split(':')
            try:
                msecs = (int(s[0]) * 3600 + int(s[1]) * 60 + int(s[2])) * 1000
        ...
        for match in re.finditer(hhmmss_sss, transcription):
            # Format #00:12:34.567#  (also .5 / .56: pad to milliseconds, as the coder does) <- L
            stamp = match.group()[1:-1]
            s = stamp.split(':')
            s2 = s[2].split('.')
            try:
        ...
        for match in re.finditer(srt, transcription):
            ...
            try:
                ...
                pass
        # Consumers (transcript scroll, click-to-seek) assume ascending text positions <- L
    ...
    def stop(self):
        ...
        self.ui.horizontalSlider.blockSignals(False)
        # Clear segment / loop playback state, otherwise the old loop resumes on next play <- L
        self.segment_play_start = None
        self.segment_play_end = None
        self.segment_loop = False
    ...
    def update_ui(self):
        ...
        if self.segment_play_end is not None and msecs >= self.segment_play_end:
            if self.segment_loop and self.segment_play_start is not None:
                media = self.mediaplayer.get_media()
                if media is None:  # released between timer ticks (e.g. while closing) <- L
                    return
    ...
    def reject(self):
        ...
        closeEvent stops playback and saves the transcript. <- L """
        return
    ...
    def update_positions(self):
        ...
        if not extending:
            ...
            for c in self.annotations:
                ...
                # table name fixed: 'annotation', not 'annotations'. <- L
