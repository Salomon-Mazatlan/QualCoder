...
class DialogReferenceManager(QtWidgets.QDialog):
    ...
    def _import_pdf_annotations(self, fid, file_path, fulltext, progress=None):
        """
        Annotations, highlights and underlines of an attached PDF, treated exactly as in
        Manage files: text annotations go to the file memo and, if the user agrees (asked
        once per import session), highlights are coded under the PDF Highlights category
        and underlines under PDF Underlines. Uses the shared pdf_utils helpers, so a PDF
        entered through a reference ends up like one entered through Manage files.
        """
