...
from .view_av_waveform import waveform_backend_available, WaveformSeekBar, LineNumberArea, generate_waveform_png_async, \
    waveform_colour  # noqa: F401  (WaveformSeekBar used via the promoted .ui widget) <- L
...
class DialogCodeAV(QtWidgets.QDialog):
    ...
    def __init__(self, app, parent_text_edit, tab_reports):
        ...
        self.undo_deleted_av_mirrors = []  # Wave segments removed as text-coding mirrors, for symmetric undo. <- L
        self.undo_deleted_text_mirrors = []  # Text codings removed when deleting a segment in mirror mode. <- L
        self.codes = []
        self.categories = []
        ...
        # All splitters persist their layout, as in code_text. <- L
        ...
        # Volume popup: icon button opening a vertical slider. <- L
        ...
        # The coder icon replaces the "Coder:" text label. <- L
        ...
        # Line numbers in the transcript, as in the transcription area. <- L
        LineNumberArea.install(self.ui.plainTextEdit)
        ...
        # Top coder group as in code_text: label, name lineEdit and edit-coders button. <- L
        self.ui.lineEdit_coder.setText(self.app.settings['codername'])
        self.ui.pushButton_coder.clicked.connect(self.edit_coder_names)
        # Autocode, show annotations and show memos next to the coder; search row below. <- L
        ...
        # F2-F6 shortcuts and category branch deletion. <- L
        ...
        # Restore the page-specific entry lost in the tree migration. <- L
        ...
        # The .ui carries generous minimums so Qt Designer opens spacious; release
        # them at runtime to keep the main window freely shrinkable. <- L
        ...
        try:
            ...
            # These keys once stored HEIGHTS (old vertical orientation); applied as
            # widths they leave the AV panel ~300 px and the media buttons clipped.
            # Only restore plausible widths for the AV pane. <- L
            if v0 > 450 and v1 > 10:
            ...
            # the default sizeHint left it ~300 px. <- L
            self.ui.splitter_right.setSizes([620, 420])
        # Default split: two thirds AV area, one third transcript. <- L
        ...
        # tinted waveform, full width like the slider. <- L
        ...
        # waveform: the playhead lands on the same x in both. <- L
        ...
        # Classic position slider above the waveform, as in the original design. <- L
        self.ui.horizontalSlider.setEnabled(False)
    ...
    def show_memos(self):
        """ Show all memos for coded transcript text in a dialog, as in code_text. <- L """
    ...
    def show_annotations(self):
        """ Show all annotations for the transcript text in a dialog, as in code_text. <- L """
    ...
    def auto_code(self):
        ...
        pipe | splits multiple find texts. Port of the code_text auto_code. <- L """
    ...
    def _record_speakers_undo(self, ctids_before):
        ...
        new code_text rows and their mirrored wave segments. <- L """
    ...
    def undo_autocoding(self):
        """ Present a list of choices for the undo operation, as in code_text. <- L """
        ...
        self.get_coded_text_update_eventfilter_tooltips()
        self.fill_code_counts_in_tree()
        # Refresh the wave: undo may have deleted mirrored speaker segments. <- L
    ...
    def reset_search_state(self):
        ...
        point into the previous text. <- L """
    ...
    def search_for_text(self):
        ...
        Three or more characters. Ported from view_av. <- L """
    ...
    def move_to_previous_search_text(self):
        """ Move to previous search match in the transcript. <- L """
    ...
    def move_to_next_search_text(self):
        """ Move to next search match in the transcript. <- L """
    ...
    def _select_search_result(self):
        """ Select the current search match in the transcript and update the totals label. <- L """
    ...
    def edit_coder_names(self):
        """ Open the coder names dialog, as in code_text. <- L """
    ...
    def update_coder_names(self):
        ...
        transcript codings and tree counts are all filtered by codername. <- L """
    ...
    def mark_speakers(self):
        ...
        on accept, refresh codes and codings and offer to show the speaker coder. <- L """
        ...
        # Snapshot to make this run undoable via autocode undo. <- L
        ...
        if ui_speaker.exec() == QtWidgets.QDialog.DialogCode.Accepted:
            ...
            # With sync on, mirror the new speaker codings onto the wave (transcript timestamps). <- L
    ...
    def video_frame_menu(self, position):
        ...
        # Primary action saves the frame into the project; export to file is secondary. <- L
    ...
    def _set_video_output(self):
        ...
        if self.video_window is not None:
            winid = int(self.video_window.dframe.winId())
        else:
    ...
    def _strip_tree_color_icons(self):
        """ No colour chips before code names in this dialog's tree. <- L """
    ...
    def fill_code_counts_in_tree(self):
        ...
        cur = self.app.conn.cursor()
        code_counts = []
        # Counts for all VISIBLE coders (segments and text), matching wave and transcript. <- L
        ...
        for c in self.codes:
            ...
            # Media without a transcript: transcription is None, count no text codings <- L
        ...
        # (a sub-code is attributed to its top ancestor's category). <- L
    ...
    def load_media(self):
        ...
        if self.file_['av_text_id'] is not None:
            ...
            if self.transcription is not None and self.transcription[1] is None:
                # Old projects can hold NULL fulltext; normalise so setPlainText/regex do not crash <- L
    ...
    def _visible_coders(self):
        """ Visible coders (always including the current one), as in code_text. <- L """
        try:
    ...
    def get_coded_text_update_eventfilter_tooltips(self):
        ...
        if self.transcription is None:
            return
        # Coded text for all VISIBLE coders (code_text_visible view), as in code_text. <- L
    ...
    def get_timestamps_from_transcription(self):
        ...
        mmss1 = r"\[[0-9]{1,3}:[0-9][0-9]\]"  # up to 999 mins: the inserted [mm:ss] uses total minutes <- L
        hhmmss1 = r"\[[0-9][0-9]:[0-9][0-9]:[0-9][0-9]\]"
        mmss2 = r"\[[0-9]{1,3}\.[0-9][0-9]\]"  # <- L
        ...
        # with mixed timestamp formats the per-pattern passes are appended out of order <- L
    ...
    def _delete_linked_av_segment(self, item):
        ...
        if avid:
            cur = self.app.conn.cursor()
            # Capture the row first so Ctrl+Z can restore the band together with the text code <- L
    ...
    def _delete_av_segment_from_text_code(self, cid, pos0_char, pos1_char, text_key=None):
        ...
        def _capture(where_sql, params):
            """ Stash the rows about to be deleted, for symmetric Ctrl+Z restore. <- L """
            if text_key is None:
                return
            cur.execute("select id, pos0, pos1, cid, memo, date, owner, important "
    ...
    def slider_seek(self, value):
        ...
        Routed through seek_to_ms so slider, playhead and time label stay in sync. <- L """
    ...
    def sync_position_slider(self, msecs, duration):
        """ Reflect the playhead on the classic slider without re-triggering a seek. <- L """
        ...
        if duration is None or duration <= 0:
            return
        # Clamp: VLC/bookmarks can briefly report out-of-range msecs (int32 overflow) <- L
    ...
    def track_bar_clicked(self, seg):
        ...
        waveform) and moves the playhead to its start. <- L """
    ...
    def _mark_wave_selection_with_new_code(self):
        ...
        transcript 'Mark with new code' does (no keyboard shortcut). <- L """
    ...
    def segment_add_additional_code(self, segment):
        ...
        (codes already on the exact span are excluded). <- L """
    ...
    def _waveform_media_abs_path(self):
        """ Absolute path of the current media file, for waveform generation. <- L """
        abs_path = ""
        if self.file_['mediapath'][0:6] in ('/audio', '/video'):
    ...
    def get_waveform(self):
        ...
        # Cancel a pending generation poll from a previously loaded file <- L
        ...
        if os.path.exists(waveform_path):
            ...
            if pm.isNull():  # unreadable/corrupt image <- L
        ...
        if not waveform_backend_available():
            ...
            return
        # Worker thread build; a QTimer polls for completion in the GUI thread. <- L
        sb.set_waveform_pixmap(None)
    ...
    def show_volume_popup(self):
        """ Show the vertical volume slider above the volume button. <- L """
    ...
    def go_to_bookmark(self):
        ...
        time.sleep(0.2)
        self.mediaplayer.set_time(result[1])
        self.ui.widget_seekbar.set_position(result[1])  # the bar works in absolute msecs <- L
    ...
    def assign_segment_to_code(self, selected):
        ...
        if selected is None or selected.text(1)[0:3] != 'cid':
            # Defence in depth: never insert a catid (or nothing) as a code id <- L
    ...
    def _underline_contrast_color(self, color):
        ...
        background (lightened or darkened, keeping the hue). <- L """
    ...
    def set_highlight_style(self, style):
        ...
        in the key shared with code_text. <- L """
    ...
    def highlight(self):
        ...
        for item in self.code_text:
            ...
            if self.highlight_style == 'underline':
                # Dashed underline in the code colour, contrast adjusted. <- L
    ...
    def textedit_menu(self, position):
        ...
        # annotate/copy/AI, with the A/V specific items last. <- L
        if selected_text != "":
        ...
        if selected_text != "":
            ...
            # AI text analysis submenu, as in code_text. <- L
        ...
        # Highlight style: offer the one NOT active, as in code_text. <- L
    ...
    def change_code_to_another_code(self, position):
        ...
        try:
            ...
            # Keep the mirrored wave segment consistent with the new code. <- L
            ...
            # Do not fail silently: it made "Replace code" look broken. <- L
            self.app.conn.rollback()
    ...
    def copy_selected_text_to_clipboard(self, metadata=False):
        ...
        and reference), as code_text does; the file here is the transcript. <- L """
    ...
    def mark_with_new_code(self, in_vivo=False):
        ...
        in_vivo the selection itself is the code name (code_text port). <- L """
    ...
    def restore_unmarked_segment(self):
        ...
        try:
            ...
            # segment). Prevents silent duplicates in code_av. <- L
    ...
    def restore_unmarked_text_codes(self):
        ...
        if not self.undo_deleted_codes:
            return
        cur = self.app.conn.cursor()
        # Mirror wave segments captured by the unmark helpers, grouped per text coding <- L
        mirrors_by_key = {}
        ...
        try:
            for item in self.undo_deleted_codes:
                ...
                # Prevents IntegrityError on the code_text UNIQUE constraint. <- L
                cur.execute("select 1 from code_text where cid=? and fid=? and pos0=? and pos1=? and owner=?",
                ...
                for row in mirrors_by_key.pop(key, []):
                    ...
                    if cur.fetchone() is not None:
                        continue  # mirror segment already present, avoid duplicating it <- L
                    cur.execute("insert into code_av (id, pos0, pos1, cid, memo, date, owner, important) "
                                "values(?,?,?,?,?,?,?,?)", list(row))
                    cur.execute("select last_insert_rowid()")
                    new_avid = cur.fetchone()[0]
            ...
            # against a partially applied transaction. <- L
            self.undo_deleted_codes = []
    ...
    def unmark(self, location):
        ...
        if self.transcription is None or self.ui.plainTextEdit.toPlainText() == "":
            return
        unmarked_list = []
        # All visible codings (e.g. Mark speakers), as in code_text. <- L
        for item in self.code_text:
        ...
        self.undo_deleted_av_mirrors = []  # will be filled by the mirror-delete helpers <- L
    ...
    def segment_add_code_from_tree(self, segment):
        ...
        Mirrors SegmentGraphicsItem.add_code so the wave bands offer the same options. <- L """
        ...
        cid = int(selected.text(1).split(":")[1])
        cur = self.app.conn.cursor()
        # Avoid an exact duplicate coding (same code, file, span, coder) <- L
        cur.execute("select 1 from code_av where id=? and pos0=? and pos1=? and cid=? and owner=?",
    ...
    def change_segment_code(self, segment):
        ...
        as code_text does (it used to require selecting the code in the tree). <- L """
        ...
        cur.execute("update code_av set cid=?, date=? where avid=?",
                    [cid, datetime.datetime.now().astimezone().strftime("%Y-%m-%d %H:%M:%S"),
                     segment['avid']])
        # Keep the mirrored text coding(s) consistent with the new code. <- L
        try:
    ...
    def delete_segment(self, segment):
        ...
        if not ok:
            return
        tmp_seg = deepcopy(segment)  # the deleted segment, not the selection dict <- L
        ...
        if getattr(self, 'text_to_av_coding', True):
            # Capture the linked text codings first so Ctrl+Z restores them too. <- L
    ...
    # Reposition active handles to the code's current pos0/pos1 without recreating them. <- L
...
class SegmentGraphicsItem(QtWidgets.QGraphicsLineItem):
    ...
    def replace_code(self):
        """ Change code via the selection dialog (code_text style). <- L """
    ...
    def export_segment(self):
        ...
        # -y avoids ffmpeg hanging on its overwrite prompt (stdin is not attached). <- L
    ...
    def delete(self):
        ...
        self.code_av_dialog.undo_deleted_text_mirrors = []  # This path only unlinks text codings <- L
