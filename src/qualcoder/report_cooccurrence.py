...
class DialogReportCooccurrence(QtWidgets.QDialog):
    ...
    # --- ADDED: EXPORT BASIC GRAPH TO IMAGE ---
    def view_graph(self):
        """ Exports a high-resolution image of the co-occurrence graph """
        if not HAS_NETWORK_LIBS:
            Message(self.app, _("Missing Libraries"), _("'networkx' and 'matplotlib' are required to export graphs.")).exec()
            return
        ...
        for row in visible_indices:
            for col in visible_indices:
                ...
                if count > 0 and row != col: 
                    G.add_edge(self.selected_codes[row]['name'], self.selected_codes[col]['name'], weight=count)
        ...
        nx.draw_networkx_nodes(G, pos, ax=ax, node_size=800, node_color='#81BEF7', alpha=0.9, edgecolors='gray')
        nx.draw_networkx_edges(G, pos, ax=ax, width=normalized_weights, edge_color='#A4A4A4', alpha=0.7)
        nx.draw_networkx_labels(G, pos, ax=ax, font_size=10, font_family="sans-serif", font_weight='bold')
        nx.draw_networkx_edge_labels(G, pos, ax=ax, edge_labels=edge_labels, font_size=9, font_color='darkred')
    ...
    # --- EXPORTAR MAPA DE CLUSTERS A IMAGEN ---
    def view_cluster_graph(self):
        """ Exporta una imagen del mapa de códigos agrupados por comunidades. """
        if not HAS_NETWORK_LIBS:
            Message(self.app, _("Missing Libraries"), _("'networkx' and 'matplotlib' are required for cluster analysis.")).exec()
            return
        ...
        for i in visible_indices: G.add_node(self.selected_codes[i]['name'])
        ...
        if G.number_of_nodes() == 0 or G.number_of_edges() == 0: return
        ...
        for u, v, d in G.edges(data=True):
            if d['weight'] >= mean_w: G_strong.add_edge(u, v, weight=d['weight'])
        ...
        try:
            if nx.is_connected(G): pos = nx.kamada_kawai_layout(G, weight='distance')
            else: pos = nx.spring_layout(G, k=1.2, weight='weight', iterations=80)
        except Exception:
        ...
        nx.draw_networkx_nodes(G, pos, ax=ax, node_size=800, node_color=node_colors, alpha=0.9, edgecolors='gray')
        nx.draw_networkx_edges(G, pos, ax=ax, width=normalized_weights, edge_color='#A4A4A4', alpha=0.5)
        nx.draw_networkx_labels(G, pos, ax=ax, font_size=10, font_family="sans-serif", font_weight='bold')
        nx.draw_networkx_edge_labels(G, pos, ax=ax, edge_labels=edge_labels, font_size=9, font_color='darkred')
    ...
    def table_menu(self, position):
        ...
        action_view_graph = menu.addAction(qta.icon('mdi6.image'), "Export Co-occurrence Graph (Image)")
        
        # ADDED: Option for Cluster Analysis
        action_view_cluster = menu.addAction(qta.icon('mdi6.image-multiple'), "Export Cluster Graph (Image)")
