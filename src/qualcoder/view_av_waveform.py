...
class LineNumberArea(QtWidgets.QWidget):
    """ Line-number gutter for QTextEdit/QPlainTextEdit. Install with
    LineNumberArea.install(editor). <- L """

    def __init__(self, editor):
        super().__init__(editor)
        self.editor = editor
        doc = editor.document()
        doc.blockCountChanged.connect(self.update_width)
        ...
        if isinstance(editor, QtWidgets.QPlainTextEdit):
            editor.updateRequest.connect(lambda _r, _dy: self.update())
        self.update_width()

    @staticmethod
    def install(editor):
        area = LineNumberArea(editor)
        editor._line_number_area = area
        original_resize = editor.resizeEvent

        def _resize(event):
            original_resize(event)
            cr = editor.contentsRect()
            area.setGeometry(QtCore.QRect(cr.left(), cr.top(), area.gutter_width(), cr.height()))
    ...
    def update_width(self, _count=0):
        self.editor.setViewportMargins(self.gutter_width(), 0, 0, 0)
        cr = self.editor.contentsRect()
        self.setGeometry(QtCore.QRect(cr.left(), cr.top(), self.gutter_width(), cr.height()))
        self.update()

    def paintEvent(self, event):
        painter = QtGui.QPainter(self)
        base = self.palette().color(QtGui.QPalette.ColorRole.Base)
        painter.fillRect(event.rect(), base.darker(104) if base.lightness() >= 128 else base.lighter(120))
        text_col = self.palette().color(QtGui.QPalette.ColorRole.Text)
        text_col.setAlpha(140)
        ...
        if isinstance(self.editor, QtWidgets.QPlainTextEdit):
            # QPlainTextEdit scrolls by blocks, not pixels: use its visible-block API. <- L
            block = self.editor.firstVisibleBlock()
            while block.isValid():
                geo = self.editor.blockBoundingGeometry(block).translated(self.editor.contentOffset())
                top = int(geo.top())
                if top > viewport_h:
                    break
                if block.isVisible():
                    painter.drawText(0, top, self.width() - 5, fm.height(),
                                     int(QtCore.Qt.AlignmentFlag.AlignRight), str(block.blockNumber() + 1))
                block = block.next()
        else:
            ...
            while block.isValid():
                ...
                if top > viewport_h:
                    break
                if top + rect.height() >= 0 and block.isVisible():
                    painter.drawText(0, top, self.width() - 5, fm.height(),
                                     int(QtCore.Qt.AlignmentFlag.AlignRight), str(number))
                number += 1
                block = block.next()
...
class WaveformSeekBar(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Track view: one lane per code with a left header column; no text on the bands. <- L
        ...
        # Lanes can be hidden when an external SegmentTracksView shows the bands. <- L
        ...
        # Resize handles clear when focus moves elsewhere. <- L
    ...
    def set_track_view(self, value):
        """ Switch between the track view (lanes per code) and the flat stacked view. <- L """
    ...
    def set_lanes_visible(self, value):
        ...
        selection, playhead and segment right-click on the wave remain. <- L """
    ...
    def set_left_inset(self, px):
        """ Left inset so the wave timeline aligns with an external tracks widget. <- L """
    ...
    def set_right_inset(self, px):
        ...
        and both playheads stay aligned. <- L """
    ...
    def _x0(self):
        """ Left edge of the timeline area (after the header column in track view). <- L """
    ...
    def _ms_to_x(self, ms):
        if self._duration_ms <= 0 or self.width() <= 0:
            return 0
        # VLC can report out-of-range positions: clamp to avoid Qt int32 overflow in drawLine. <- L
    ...
    def _pack_lanes(self):
        ...
        segments of the SAME code overlap; collapsed codes draw in a single row. <- L """
        self._tracks = []
        self._lane_count = 1
    ...
    def _segment_band_rect(self, s):
        """ Screen rect of a segment's band in the current view mode. <- L """
    ...
    def mousePressEvent(self, event):
        ...
        # Header column: a click on a code header collapses/expands that lane. <- L
    ...
    def paintEvent(self, event):
        ...
        # Coded segment bands
        lanes_top = self._top_pad + self._wave_height
        lane_h = self._lane_h()
        ...
        for s in self._segments:
            ...
            # rect is the tinted wave span and names leaked onto the waveform). <- L
...
# Serialise ffmpeg runs so a bulk import does not spawn many processes at once <- L
...
class SegmentTracksView(QtWidgets.QWidget):
    ...
    span. Lives inside a QScrollArea. <- L """
    ...
    def __init__(self, parent=None):
        ...
        # collapsing branches as in the tree. <- L
        self._codes_struct = []
        self._cats_struct = []
    ...
    def set_code_structure(self, codes, categories):
        ...
        tree and allow collapsing branches. <- L """
    ...
    def _ordered_blocks(self):
        ...
        segments. Returns blocks ('cat', cat, depth) / ('code', code, depth, cids). <- L """
        groups = {}
        for s in self._segments:
            groups.setdefault(s.get('cid'), []).append(s)
    ...
    def _ms_to_x(self, ms):
        if self._duration_ms <= 0:
            return 0
        ms = max(0, min(int(ms), self._duration_ms))  # clamp: avoids int32 overflow <- L
    ...
    def mousePressEvent(self, event):
        ...
        if track is not None and y <= track['top'] + self._label_h:
            # Click on the name line collapses/expands the code, or the branch for a category. <- L
    ...
    def paintEvent(self, event):
        ...
        for t in self._tracks:
            ...
            if t.get('kind') == 'cat':
                # Category header: chevron + italic path, no chip; its summary row below. <- L
                painter.setPen(QtGui.QPen(text_col))
                ...
                # No colour chip before the code name (user preference). <- L
                painter.setPen(QtGui.QPen(text_col))
...
def _pyav_render(media_path, out_path, colour, width=1020, height=100):
    """ Decode audio with PyAV and paint a per-column min/max peak PNG matching the
    ffmpeg showwavespic look (transparent background). Peaks are collected per
    1024-sample block (tiny memory even for hours) and mapped to columns at the
    end, so no duration estimate is needed. Ints are normalised by their dtype
    full-scale range, NOT per-frame (that painted a solid block). <- L """
    ...
    for frame in container.decode(stream):
        ...
        if kind == 'i':
            import numpy as _np
            samples = samples.astype('float32') / float(_np.iinfo(samples.dtype).max)
        elif kind == 'u':
            import numpy as _np
            half = float(_np.iinfo(samples.dtype).max) / 2.0
            samples = (samples.astype('float32') - half) / half
    ...
    for x in range(width):
        ...
        mn = min(block_mins[b0:b1])
        mx = max(block_maxs[b0:b1])
        mn = max(-1.0, min(1.0, mn))
...
def waveform_backend_available():
    """ True if any waveform backend can run: PyAV, or external ffmpeg. <- L """
...
def generate_waveform_png(media_path, out_path, colour, timeout=30):
    ...
    metacharacters are safe. Returns True if the image exists afterwards. <- L """
    ...
    # never see a half-written image (atomic on the same filesystem). <- L
    tmp_path = out_path + ".tmp.png"
...
def generate_waveform_png_async(media_path, out_path, colour, timeout=30):
    ...
    Returns the Thread; poll thread.is_alive() (e.g. with a QTimer) or fire-and-forget. <- L """
