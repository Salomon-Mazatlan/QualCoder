...
# Shared PDF helpers: text extraction with the word map used by the PDF coding
# view, markup (highlight/underline) detection, colour matching against the
# QualCoder palette, and coding of markups on import. Kept in a small module so
# manage_files and manage_references do not need to import code_pdf (and its AI
# chain) for these tasks.
...
# Known highlighter palettes mapped directly to (palette hex, family name).
# Plain RGB distance mismatches some of these: Zotero purple #A28AE5 lands on a
# blue shade and Acrobat's pale yellow #FCF485 on the golden ramp (named orange
# by colour_ranges), so the exact colours of Zotero and Acrobat are pinned here.
KNOWN_MARKUP_COLORS = {
...
def extract_pdf_highlights(filepath):
    ...
    Returns a list of {'page': page index, 'quads': [pymupdf.Rect, ...] in ROTATED
    page coordinates (the same space as the extractor's word rects),
    'color': '#RRGGBB', 'kind': 'highlight' or 'underline', 'memo': comment text}.
    Empty list when the PDF has no markups or cannot be read.
    """
...
def extract_pdf_annotations(filepath):
    """
    Non-markup annotations WITH text content (sticky notes, free text, comments
    on strikeout, etc.), for appending to the file memo on import.
    Highlight and underline comments are NOT included here: they go to the memo
    of their coded segment when markup coding is accepted.
...
def closest_qualcoder_color(hex_color):
    """
    Matches a markup colour to the QualCoder palette.
    Exact colours of known highlighter palettes (Zotero, Acrobat) map directly via
    KNOWN_MARKUP_COLORS. Achromatic colours (white/black/grays) are matched inside
    the gray range, because plain RGB distance lands them on odd hues. Every other
    colour goes through color_selector.color_matcher().
    Returns (hex of the palette colour, family name from colour_ranges).
    """
    ...
    if max(r, g, b) - min(r, g, b) < 32:
        # Achromatic: nearest shade of the gray range, same metric as color_matcher.
...
def pdf_annotations_to_file_memo(app, parent_text_edit, fid, filepath, existing_memo=""):
    """
    Appends the PDF's non-markup annotations (the ones carrying text) to the
...
def pdf_markups_question_text(highlights):
    """
    Builds the once-per-batch question shown before coding imported PDF markups,
    naming only the categories that will actually be used. Shared by manage_files
    and manage_references so both dialogs ask with the same wording.
    Args:
        highlights: output of extract_pdf_highlights
    Returns:
        String
...
def _markup_category_id(app, kind, owner, now_):
    """ Returns the catid for the markup kind, creating the category once. """
...
def code_pdf_highlights(app, parent_text_edit, fid, filepath, fulltext, highlights,
    ...
    """
    Codes the PDF's marked segments: creates (or reuses) the 'PDF Highlights'
    category for highlight annotations and the 'PDF Underlines' category for
    underline annotations, with one code per distinct colour and kind, named and
    coloured after the closest QualCoder palette colour ("Highlight yellow",
    "Underline red", "_2" suffix for further shades), then inserts code_text rows
    over the exact marked positions.
    ...
    cur = app.conn.cursor()
    # One category per kind present, created once and reused across batches.
    catids_by_kind = {}
    for kind in sorted({p['kind'] for p in positions}):
        catids_by_kind[kind] = _markup_category_id(app, kind, owner, now_)
    # One code per distinct colour and kind, named by family ("Highlight yellow",
    # "Underline red", "_2" suffix for further shades); reused if the palette
    # colour exists under that name pattern.
    cids_by_key = {}
    ...
    for seq, pos in enumerate(positions, start=1):
        ...
        if seltext.strip() == "":
            continue
        try:
            # The comment attached to the markup (if any) becomes the memo of
            # this coded segment.
